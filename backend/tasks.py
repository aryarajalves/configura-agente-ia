"""
tasks.py — Background tasks registered with the TaskIQ broker.

Replaces the legacy Celery-based tasks. All tasks are now native async
functions decorated with ``@broker.task`` and receive a SQLAlchemy session
via the TaskIQ worker state (lifecycle hooks in broker.py).
"""
...
async def _delete_old_logs(days: int = 30) -> int:
    async with async_session() as db:
...
@broker.task(
    task_name="delete_old_process_logs_task",
    schedule=[{"cron": "0 0 * * *"}],   # Every day at midnight
)
async def delete_old_process_logs_task() -> dict:
    """Periodic task: remove process logs older than 30 days."""
    deleted = await _delete_old_logs(days=30)
    logger.info("Scheduled cleanup: deleted %d old process logs.", deleted)
...
@broker.task(task_name="process_video_task")
async def process_video_task(log_id: int, payload: dict) -> None:
    """
    Task para processamento de vídeo e geração de bases de conhecimento
    usando AssemblyAI e IA local.
    """
    ...
    try:
        ...
        metadata_str = (
            " | ".join(f"{k}: {v}" for k, v in md_dict.items())
            if md_dict
        ...
        if s3_service.enabled and video_path and not os.path.exists(video_path):
            ...
            logger.info("Usando URL pré-assinada para S3 (video_task): %s", video_path)
        ...
        await _update_log_status(
            log_id, "PROCESSANDO", 40, details_update={"transcription": "Concluída"}
        )
            ...
            async with async_session() as db:
                ...
                if options.get("gerar_resumo"):
                    try:
                                ...
                                messages=[
                                    {
                                        "role": "system",
                                        "content": (
                                            "Você é um assistente especialista em síntese "
                                            "de informações. Resuma o texto fornecido em "
                                            "pontos principais (bullet points) de forma "
                        ...
                        logger.error("Erro no resumo process_video_task: %s", e)
                ...
                for i in range(0, total_items, batch_size):
                    ...
                    batch_texts = [
                        f"{item.metadata_val} | {item.question} | {item.answer}"
                        for item in batch
                    ]
        ...
        logger.error("Erro no process_video_task %d: %s", log_id, str(e))
        ...
        if video_path:
            if os.path.exists(video_path):
                ...
                try:
                    s3_service.delete_object(video_path)
                except Exception:
                    pass
...
# ---------------------------------------------------------------------------
# KB JSON Item Processing
# ---------------------------------------------------------------------------

@broker.task(task_name="process_kb_json_item_task")
async def process_kb_json_item_task(log_id: int, kb_id: int, payload: dict) -> None:
    """
    ...
    try:
        ...
        if not text_content.strip():
            await _update_log_status(
                log_id, "ERRO", 0, error_message="Conteúdo vazio para este item."
            )
            return
        ...
        async with async_session() as db:
            ...
            if options.get("extractQA"):
                ...
                qa_list, _ = await generate_global_qa(
                    text_content, total_questions=num_q
                )
                if qa_list:
                    for item in qa_list:
                        items_to_add.append(
            ...
            if options.get("generateSummary"):
                try:
                    ...
                    if client:
                        ...
                        items_to_add.append(
                            KnowledgeItemModel(
                                knowledge_base_id=kb_id,
                                question=(
                                    f"Resumo - {metadata_val}"
                                    if metadata_val
                                    else "Resumo de Item JSON"
                                ),
                                answer=summary_text,
                    ...
                    logger.error("Erro no resumo JSON task: %s", e)
            ...
            if options.get("extractChunks"):
                ...
                c_overlap = options.get("chunkOverlap", 150)
                chunks = chunk_text(
                    text_content, chunk_size=c_size, overlap=c_overlap
                )
                for i, chunk in enumerate(chunks):
                    items_to_add.append(
                        KnowledgeItemModel(
            ...
            for i in range(0, total_items, 50):
                ...
                await _update_log_status(
                    log_id,
                    "PROCESSANDO",
                    60 + int(((i + len(batch)) / total_items) * 35),
                )
            ...
            await _update_log_status(
                ...
                "CONCLUIDO",
                100,
                details_update={"items_generated": total_items},
            )
            return total_items

    except Exception as e:
        logger.error("Erro no process_kb_json_item_task %d: %s", log_id, str(e))
        await _update_log_status(log_id, "ERRO", 0, error_message=str(e))
    finally:
        await engine.dispose()
